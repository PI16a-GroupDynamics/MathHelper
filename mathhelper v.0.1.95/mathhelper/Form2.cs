...
namespace mathhelper
{
    public partial class Form2 : Form
    {
        ...
        {
            richTextBox1.Clear(); //очистка rtb
            double res = 0;
            double S = 0;
            string str = ""; //для вывода значений в rtb
            int n; //хранит количество знаков, нужных для округления чисел
            int rnd; //round округляет только для 15 знаков!

            n = Convert.ToInt32(textBox1.Text);
            rnd = Convert.ToInt32(textBox2.Text);
            if (comboBox1.SelectedIndex == 0) //если выбрали первую
            {
                for (int i = 1; i <= n; i++)
                {
                ...
                richTextBox1.AppendText(Environment.NewLine); //переход на новую строчку
                res = S = 0;
            }
            else if (comboBox1.SelectedIndex == 1) //если выбрали вторую
            {
                for (int i = 1; i <= n; i++)
                {
            ...
            else if (comboBox1.SelectedIndex == 2) //если выбрали третью
            {
                for (int i = 1; i <= n; i++)
                {
            ...
            var document = new DocumentModel(); //для pdf
        ...
        private void журналПользователяToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            jrnl.Show(); //открывает журнал пользователя
        }

        private void Form2_Load(object sender, EventArgs e)
        {
            ToolTip t = new ToolTip(); //высплывающая подсказка для 
        ...
        {
            Help help = new Help(); //вызов формы со справкой
            help.Show();
        }

        private void историяИзмененийToolStripMenuItem_Click(object sender, EventArgs e)
        {
            changelog chnglg = new changelog(); //вызов формы с историей изменений приложения
            chnglg.Show();
        }

        private void развернутьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            WindowState = FormWindowState.Normal;//разворачивает окно
        }
